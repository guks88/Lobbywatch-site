var initVegaMoyenneBranche = function(affairVoteId) {
    var spec =
    {
...
var getAverage = function(affairVoteId, spec) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#moyenneGenerale').append('<p style="text-align: center">Moyenne générale sur toutes les branches</p>');

            var view = new vega.View(vega.parse(spec), {
                loader: vega.loader({baseURL: ''}),
                logLevel: vega.Warn,
                renderer: 'canvas'
            }).initialize('#moyenne_chart_branche').hover().run();
        }
    });
};

var chartBranchesEtVotesUpdate = function(affairVoteId, selected){
    var spec =
    {
        "$schema": "https://vega.github.io/schema/vega/v3.0.json",
        "width": 500,
...
var getAverageOfSelected = function(affairVoteId, selected, spec){
    ...
    $.ajax({
        url: "/get_average_vote_of_branche_selected",
        ...
        success: function (result) {
            ...
            $('#moyenneGenerale').append('<p style="text-align: center">Moyenne pour la branche : ' + nomBranche + '</p>');
